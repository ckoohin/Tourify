export const validateBooking = (data, quotesSelected = true) => {
    const errors = {};
    ...
    if (!quotesSelected) {
        errors.quotesSelected = "Vui lòng chọn báo giá";
    }
    ...
    numberFields.forEach((field) => {
        ...
        if (Number(data[field]) < 0) {
            errors[field] =
                "Vui lòng nhập số lượng hành khách hợp lệ (lớn hơn 0)";
        }
    });

    // Validate Số tổng số lượng khách
    if (Number(data.total_guests) <= 0) {
        errors.total_guests = "Vui lòng chọn số lượng hành khách";
    }
